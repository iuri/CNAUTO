...
ad_form -extend -name vehicle_one -on_request {
	...
	SELECT cv.vin, cc.pretty_name AS model, cr.pretty_name AS resource, cv.engine, cv.year_of_model, cv.year_of_fabrication, cp.pretty_name AS owner, cp2.pretty_name AS distributor, cv.purchase_date, cv.arrival_date, cv.billing_date  
