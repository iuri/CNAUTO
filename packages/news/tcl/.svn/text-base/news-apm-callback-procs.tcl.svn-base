...
ad_library {
    ...
    @cvs-id $Id: news-apm-callback-procs.tcl,v 1.1.1.1 2006/05/13 17:20:50 tekne Exp $
}
...
ad_proc -private news::install::after_upgrade {
	...
	-spec {
	    ...
	    5.2.2d4 5.2.2d5 {
		rel_types::new "replicated_news" "Replicated News" "Replicated News" "content_item" 0 1 "content_item" 0 1
		
		apm_parameter_register "ReplicateNewsItemP" "Replicate news item to other mounted instances" news 0 number "" 1 1

		apm_parameter_register "PreviewNewsItemP" "Preview news item before publish it" news 0 number "" 1 1

		set package_id [db_string select_package_id {
		    SELECT object_id FROM site_nodes WHERE parent_id = (SELECT node_id FROM site_nodes WHERE name = 'portal') AND name = 'noticias'
		} -default ""]

		parameter::set_value -package_id $package_id -parameter "ReplicateNewsItemP" -value 1
		parameter::set_value -package_id $package_id -parameter "PreviewNewsItemP" -value 1
